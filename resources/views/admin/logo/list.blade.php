        ...
        <main class="flex-1 p-6">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 text-gray-900">
                ...
                <div class="d-flex justify-content-between align-content-center">
                    <h1>Danh sách logo</h1>
                    <a href="{{ route('admin.logos.create') }}" class="btn btn-outline-success">Thêm mới</a>
                </div>
                <table class="table">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        @foreach ($logos as $key => $value)
                            <tr>
                                ...
                                <td>{{ $value->name }}</td>
                                <td>
                                    <img src="{{ asset('storage/' . $value->image) }}" alt="" width="200">
                                </td>
                                <td>
                                    {!! $value->active ? '<p class="text-success">Kích hoạt</p>' : '<p class="text-danger">Ngưng hoạt động</p>' !!}
                                </td>
                                <td>
                                    <div class="d-flex">
                                        <a href="{{ route('admin.logos.edit', $value->id) }}"
                                            class="btn btn-outline-warning me-2">Sửa</a>

                                        <form action="{{ route('admin.logos.destroy', $value->id) }}" method="POST"
                                            onsubmit="return confirm('Bạn có chắc chắn muốn xóa logo này không?');">

                                            {{-- Token bảo mật --}}
                                            @csrf

                                            {{-- Phương thức HTTP là DELETE --}}
                                            @method('DELETE')
