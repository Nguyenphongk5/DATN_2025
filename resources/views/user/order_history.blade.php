...
@section('content')
    <section class="py-10 bg-gradient-to-r from-blue-100 via-purple-100 to-pink-100 border-b mb-10 shadow-inner">
        <div class="container mx-auto px-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <h3 class="text-3xl md:text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 mb-0 drop-shadow-lg">
                Đơn hàng của tôi
            </h3>
        </div>
    </section>
                    ...
                    fetch("{{ url('/order-history/filter') }}?status=" + status)
                        ...
                        .then(html => {
                            ...
                            document.querySelectorAll('.filter-tab').forEach(t => {
                                t.classList.remove('bg-gradient-to-r', 'from-pink-500', 'to-indigo-500', 'text-white', 'scale-105', 'border-pink-400', 'shadow-xl');
                                t.classList.add('bg-white', 'text-indigo-700');
                            });
                            tab.classList.remove('bg-white', 'text-indigo-700');
                            tab.classList.add('bg-gradient-to-r', 'from-pink-500', 'to-indigo-500', 'text-white', 'scale-105', 'border-pink-400', 'shadow-xl');
                        })
                        .catch(err => {
