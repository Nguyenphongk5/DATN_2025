<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3 bg-gradient-to-r from-indigo-500 via-sky-500 to-cyan-400 rounded-2xl shadow-xl px-6 py-4 mb-6">
            <i class="fas fa-blog text-3xl text-white drop-shadow-lg"></i>
            <h2 class="font-extrabold text-2xl text-white tracking-wide drop-shadow-lg">Quản lý bài viết</h2>
        </div>
    </x-slot>

    <div class="py-8">
        <h1 class="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 via-sky-500 to-cyan-400 drop-shadow-lg flex items-center gap-2 mb-8 justify-center">
            ...
            <div class="bg-white/90 shadow-2xl rounded-3xl p-8">
                <div class="flex flex-wrap gap-4 items-end mb-8">
                    <form action="{{ route('admin.blogs.index') }}" method="GET" class="flex flex-wrap gap-4 items-end">
                        <select name="user_id" class="border border-indigo-200 rounded-xl px-4 py-2 focus:ring-2 focus:ring-sky-400 shadow">
                        ...
                        <select name="is_active" class="border border-indigo-200 rounded-xl px-4 py-2 focus:ring-2 focus:ring-sky-400 shadow">
                        ...
                        <button type="submit" class="bg-gradient-to-r from-sky-400 to-indigo-500 hover:from-indigo-500 hover:to-sky-400 text-white font-bold px-6 py-2 rounded-xl shadow-lg flex items-center gap-2 transition">
                            <i class="fas fa-filter"></i> Lọc
                        </button>
                    </form>

                    <a href="{{ route('admin.blogs.create') }}" class="bg-gradient-to-r from-green-400 to-cyan-400 hover:from-cyan-400 hover:to-green-400 text-white font-bold px-6 py-2 rounded-xl shadow-lg flex items-center gap-2 transition">
    ...
    @foreach ($blogs as $key => $blog)
        <tr class="hover:bg-gradient-to-r hover:from-indigo-50 hover:to-cyan-50 transition">
            ...
            <td class="px-6 py-4">
                ...
                @else
                    <div class="w-16 h-16 bg-gray-200 rounded-xl mx-auto flex items-center justify-center">
            ...
            <td class="px-6 py-4">
                @if ($blog->is_active === 1)
                    <span class="inline-block px-3 py-0.5 text-xs font-semibold rounded-full bg-green-100 text-green-700 border border-green-300">Đang hoạt động</span>
                @else
                    <span class="inline-block px-3 py-0.5 text-xs font-semibold rounded-full bg-gray-200 text-gray-700 border border-gray-400">Đã ẩn</span>
                @endif
            </td>
            <td class="px-6 py-4">
                <div class="flex gap-2 justify-center">
                    <a href="{{ route('admin.blogs.show', $blog->id) }}"
                        class="bg-gradient-to-r from-sky-400 to-indigo-500 hover:from-indigo-500 hover:to-sky-400 text-white font-bold py-2 px-5 rounded-xl shadow-lg flex items-center gap-2 transition text-sm">
                        <i class="fas fa-eye"></i> Xem
                    </a>
                    <a href="{{ route('admin.blogs.edit', $blog->id) }}"
                        class="bg-gradient-to-r from-yellow-400 to-pink-400 hover:from-pink-400 hover:to-yellow-400 text-white font-bold py-2 px-5 rounded-xl shadow-lg flex items-center gap-2 transition text-sm">
