            ...
            <div class="bg-white rounded-3xl shadow-2xl p-8 border border-gray-100 flex flex-col gap-4">
                ...
                <div class="space-y-3 text-gray-700 text-base">
                    ...
                    <p class="flex items-center gap-2">
                        ...
                        <span id="payment-status" class="capitalize px-2 py-1 rounded font-semibold text-green-700">
                            {{ $paymentLabel }}
                        </span>
                    </p>
                ...
                @if (in_array($order->status, ['pending']))
                    <form id="cancel-form" action="{{ route('orders.cancel', $order->id) }}" method="POST"
                        class="mt-6">
                        @csrf
                        @method('PUT')
                        <button
                ...
                @if (in_array($order->status, ['completed', 'returned']))
                    @if (!$order->returnRequest)
                        {{-- Chưa có yêu cầu -> Hiện nút Hoàn hàng --}}
                        <a href="{{ route('returns.create', $order->id) }}" id="return-btn"
                    ...
                    @elseif ($order->returnRequest->status === 'pending')
                        <div
                            class="mt-4 px-6 py-4 bg-yellow-50 border-l-4 border-yellow-400 rounded-xl text-gray-800 shadow-md">
                            <h3 class="font-bold text-lg mb-1">Thông tin hoàn hàng</h3>
                            <p><span class="text-yellow-600 font-semibold">Quý khách vui lòng đợi phản hồi từ shop.</span>
                            </p>
                        </div>
                    @else
                        <div
                            ...
                            <p><strong>Phản hồi từ shop:</strong>
                                {{ $order->returnRequest->response_note ?? 'Chưa có phản hồi' }}
                            </p>
                            <p><strong>Lưu ý:</strong>
                                Quý khách sẽ được thanh toán khi shiper đến lấy hàng
                            </p>
                        </div>
                    @endif
                @endif
