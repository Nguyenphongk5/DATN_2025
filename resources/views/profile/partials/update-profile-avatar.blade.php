<section class="mb-8">
    <div class="bg-white/90 shadow-xl rounded-2xl p-8">
        <h4 class="text-lg font-bold text-indigo-700 mb-2 flex items-center gap-2"><i class="fas fa-image"></i> Update
            Avatar</h4>
        <p class="text-gray-500 mb-4">Upload a new profile picture for your account.</p>
        <form method="POST" action="{{ route('profile.avatar') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <!-- File input -->
            <div>
                <label for="avatar" class="block text-sm font-bold mb-2 text-indigo-700">Choose new avatar</label>
                <input
            ...
            <div class="flex justify-between items-center mt-6">
                ...
                @if (session('status') === 'avatar-updated')
                    <span class="text-green-600 text-sm font-semibold ml-4 flex items-center gap-1"><i
                            class="fas fa-check-circle"></i> Avatar updated!</span>
                @endif
            </div>
        </form>
    </div>
