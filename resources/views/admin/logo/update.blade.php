<x-app-layout>
    <x-slot name="header">
        <div
            ...
            <h2 class="font-extrabold text-2xl text-white tracking-wide drop-shadow-lg">Cập nhật Logo</h2>
        </div>
    </x-slot>
    <div class="py-8">
            ...
            <div class="bg-white/90 shadow-2xl rounded-3xl p-8">
                ...
                <form action="{{ route('admin.logos.update', $logo->id) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $logo->name) }}" required
                    ...
                    <div>
                        ...
                        <select name="is_active" id="is_active"
                            class="w-full px-4 py-3 rounded-xl border border-indigo-200 focus:ring-2 focus:ring-sky-400 focus:outline-none shadow">
                            <option value="1" {{ old('is_active', $logo->is_active) == '1' ? 'selected' : '' }}>Đang
                                sử dụng</option>
                            <option value="0" {{ old('is_active', $logo->is_active) == '0' ? 'selected' : '' }}>Ẩn
                            </option>
                        </select>
                        @error('is_active')
                            <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
